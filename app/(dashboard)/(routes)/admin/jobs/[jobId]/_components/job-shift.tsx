...
export default function JobShift({ initialData, jobId }: JobShiftProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      shiftTiming: initialData.shiftTiming?.toString() || "",
    },
  });
  ...
  const selectedOption = options.find(
    (option) => option.value === initialData.shiftTiming?.toString()
  );

  return (
    <div className="mt-6 border bg-neutral-100 rounded-md p-4">
      <div className="font-medium flex items-center justify-between">
        <h2 className="font-bold text-lg text-neutral-700">Job Shift Timing</h2>
